# Progress Forge CLI User Guide

Progress Forge is an agentic software development life cycle (SDLC) command-line tool from Progress. It orchestrates AI coding agents through structured workflows that help teams plan work, implement changes, review results, and maintain an auditable record of AI-assisted development.

Progress Forge provides one command surface above the AI agents and development systems your team already uses. Project context, team conventions, workflow stages, checks, and outputs stay connected to the task instead of being scattered across chat sessions and tools.

## Why Use Progress Forge

AI coding agents can produce useful results quickly, but teams still need a consistent way to define work, preserve context, review changes, and apply engineering controls. Progress Forge adds that operating model around the agents.

Progress Forge helps your team:
...
Progress Forge coordinates supported AI coding agents through a consistent CLI. You can add or change an agent without redesigning the workflow that surrounds it.
...
Progress Forge carries relevant context into a workflow, including project metadata, issue IDs, pull request IDs, ticket IDs, technology choices, testing frameworks, and package managers. This context helps agents make decisions that match the repository and the task.
...
Progress Forge defines commands, subcommands, options, help text, and workflow operations through TOML configuration. Teams can create custom commands that match their development process instead of forcing every project into one fixed sequence.
...
Progress Forge validates configuration at startup and reports errors before a workflow runs. This makes team-defined workflows easier to test and maintain.

### Traceability and Governance

Progress Forge records workflow execution so teams can understand how an AI-assisted change was produced. Traces can capture prompts, outputs, decisions, workflow stages, code changes, and model or token usage.
...
Progress Forge connects workflows to development systems such as GitHub, Jira, Azure DevOps, Shortcut, ServiceNow, Freshdesk, and PagerDuty, depending on the configured integration. It also supports code platforms and security tools used in the development process.
...
Progress Forge includes workflow roles for activities such as product management, software architecture, software engineering, technical writing, security analysis, and site reliability engineering. Shell completions for Bash, Zsh, Fish, and PowerShell help developers discover commands and options from their terminal.

## Getting Started

Start with these sections:

1. **[Installation](./getting-started/installation.md)** - Install Progress Forge on your system
...
- **Configuration**: How to configure and customize Progress Forge
...
- **Built-in help**: Run `frg --help` or `frg <command> --help`.
- **Documentation**: Run `frg docs` to open the documentation locally.
- **Issues**: Report bugs in the project repository.
- **Discussions**: Ask questions in the project discussions.

## Version

This documentation is for Progress Forge CLI version 2.6.0.

## Next Steps

Continue with [Installing Progress Forge](./getting-started/installation.md), then follow the [Quick Start guide](./quick-start.md) to configure an agent and project metadata.
