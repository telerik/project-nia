...
| `cached_tokens` | integer | Number of tokens served from cache (cache reads, reduces cost) |
| `cache_write_tokens` | integer | Number of tokens written to cache this turn (cache creation, priced separately from reads; omitted when the agent doesn't report it) |
