...
The fastest way to learn Progress Forge is to run its built-in, guided tutorials against a real codebase. The `frg learn` command walks you through a hands-on curriculum on the [healthcare-app-angular](https://github.com/telerik/healthcare-app-angular) sample app: it validates your environment, explains what each command does, runs the real Progress Forge workflow, and tracks your progress as you go.

This guide is the quickest path from zero to productive. Each tutorial executes an actual Progress Forge command against real code and real GitHub issues, so by the end you will have seen `frg ask`, the issue-to-PR workflow, ticket triage, security review, and backlog planning in action — without writing any configuration by hand.

> **Prefer to use your own repository?** Follow the [Quick Start](./quick-start.md) instead. This sample-app guide is optimized for learning; the standard Quick Start is optimized for adopting Progress Forge in your own project.
...
The sample app includes a **Dev Container** that installs Node.js, the GitHub CLI (`gh`), supported AI coding agents, and Progress Forge automatically. This is the recommended approach because:
...
* **Progress Forge** — follow the installation steps in the standard [Quick Start](./quick-start.md).
...
Confirm Progress Forge is available:

```bash
frg --version
```

The terminal prints a Progress Forge version number, for example `frg 4.3.5`.
...
- Explains the Progress Forge configuration system.
- Prompts you to select a coding agent — **GitHub Copilot CLI**, **Claude Code**, or **OpenCode**.
- Runs `frg config init --agent <agent> --models stable` for you.
- Extracts the offline documentation to `.forge/cache/docs/`.

The `--models stable` flag configures Progress Forge to use specific, optimised models rather than automatic model selection. This ensures consistent, predictable behavior across your tutorials and workflows.

 > **Why not use automatic model selection?** While agents support an `auto` option that dynamically selects models, this can lead to inconsistent output quality and unpredictable costs. The `stable` profile provides reliable behavior that's been validated with Progress Forge's prompts. For advanced model configuration, see [AI Model Selection](./agents/model-selection.md).
...
|---|----------|---------------|----------------|-------|
| 1 | Architecture Overview | `frg ask` | Simple Q&A demonstrating code base comprehension | ~2 min |
| 2 | Developer Guide | `frg ask` | Simple Q&A requesting developer how-to documentation for the project | ~2 min |
...
| 7 | Backlog Creation | `frg backlog create` | Plan a phased implementation from your backlog | ~5 min |
...
2. **Explains what you'll learn** — the concept, the value Progress Forge adds, and links to the relevant documentation.
3. **Runs the real command** — the exact `frg` command is printed, then executed against the sample app.
4. **Tracks your progress** — completion status is saved to `.forge/config/learn.toml`, so `frg learn next` always resumes where you left off.

To jump straight to a specific tutorial instead of following the sequence, run it by name:

```bash
frg learn run ask-architecture
frg learn run issue-validation
```
...
```bash
frg learn run ask-architecture --force
```
...
- `frg learn init` configured an agent and `frg status` reports no blocking problems.
- `frg learn list` shows tutorials marked as completed.
- Running `frg learn next` reports that all tutorials are complete.

## You're ready for your own project

> **Congratulations — you are now proficient with Progress Forge.** You have used `frg ask`, the `issue-to-review-lite` and `issue-to-review` workflows, ticket triage, a long-horizon security review, and backlog planning against a real codebase. The next step is to **set up Progress Forge in your own project**: initialize configuration with `frg config init`, describe your project in `.forge/config/project.toml`, and run your first workflow on code you own.

Continue with the [Quick Start](./quick-start.md) to configure Progress Forge in your own project and automate an issue from planning to pull request with `frg workflow run issue-to-pr`.

## Next steps

- [Set up Progress Forge in your own project](./quick-start.md) with your issue tracker and code platform.
