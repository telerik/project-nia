...
meta_title: Configure Custom Agents in Progress Forge
description: Configure specialized custom agents for Progress Forge workflows with CLI, operation, target, and agent-level selection rules.
slug: custom-agent-configurations
---

# Custom Agent Configurations

Custom agent configurations select a named agent persona or configuration within the AI coding agent that Progress Forge invokes. Use them when different workflows need different expertise, such as Python development, security review, or technical writing.

Progress Forge keeps custom-agent selection separate from these settings:
...
Progress Forge passes the selected custom-agent name to the configured agent. Progress Forge does not define the custom agent's instructions or validate that the selected agent platform contains a matching configuration.

## Prerequisites

Before configuring a custom agent, verify the following requirements:

1. Install and authenticate the AI coding agent selected by Progress Forge.
2. Create the custom agent in that AI coding agent's configuration system.
3. Add the custom-agent name to `.forge/config/agents.toml` or pass it with `--custom-agent`.
...
Progress Forge resolves one custom-agent value for each workflow command. The resolution process uses this order, from highest to lowest precedence:
...
Progress Forge builds the operation key from the target and operation in the form `target.operation`, such as `issue.draft`. If an operation-specific entry does not match, Progress Forge checks the target map and then the agent-level default.

When a custom agent is selected, Progress Forge omits the built-in role prompt from the composed prompt. The custom agent supplies its own persona. If no custom agent is selected, Progress Forge includes the role prompt selected for the workflow.

## Configure Custom Agents

Store persistent settings in `.forge/config/agents.toml`. The top-level agent name must match a registered Progress Forge agent, and the `agent.default` value selects the default agent implementation.
...
- `frg issue draft` uses `issue-specialist`.
- `frg pr review` uses `pr-reviewer`.
- `frg code create` uses `code-expert`.
- `frg docs generate` uses `doc-writer`.
...
frg issue draft --custom-agent python-expert

# Use code reviewer for pull-request review
frg pr review --custom-agent code-reviewer

# Use documentation specialist for documentation generation
frg docs generate --custom-agent doc-writer

# Combine custom-agent and model selection
frg code create --custom-agent python-expert --model claude-opus-5
```
...
The `command` and `model` properties are adjacent agent settings. They do not select a custom agent, but they affect which executable and model Progress Forge uses with the selected custom-agent configuration.

Progress Forge also accepts extended target and operation objects in the configuration schema. Those objects include a `custom_agent` field, but the custom-agent resolver currently reads `custom_agents` and `custom_agent_operations` for target and operation selection. Use the dedicated custom-agent maps documented in this article until the resolver supports those extended fields.

### Use Non-Empty Names

Custom-agent names are strings. Progress Forge emits a validation warning for an empty agent-level, target-level, or operation-level name. The resolver does not verify the name against the external agent platform.

### Use Operation Keys

Operation-specific custom-agent keys must use the `target.operation` form. Progress Forge emits a configuration warning for a key that does not contain the expected target and operation structure.

## Configure the Agent Command

Use the optional `command` property to override the command or executable path that Progress Forge uses for an agent:
...
When `command` is omitted, Progress Forge uses automatic discovery for GitHub Copilot CLI. The implementation searches for `copilot` in `PATH`, handles native Windows executables, and parses supported Windows npm wrappers. The source code does not specify a universal discovery process for every registered agent.
...
Use a native executable or a supported npm installation instead. Progress Forge reports this configuration as unsupported during the installation check.
...
Progress Forge rejects an empty command and command strings containing shell operators during configuration validation.
...
| Five | None | No matching setting | Progress Forge uses no custom agent. |
...
- `frg issue draft` uses `issue-analyst`.
- `frg pr review` uses `pr-expert`.
- `frg code create` uses `coding-specialist`.
- `frg job run` has no target-specific custom agent and uses no custom agent unless `custom_agent` is also configured.
...
- `frg issue draft` uses `draft-expert` from the operation map.
- `frg issue refine` uses `issue-specialist` from the target map.
- `frg pr review` uses `general-assistant` from the agent default.
...
Progress Forge uses `my-special-agent` for that invocation, even when the configuration contains another operation, target, or default value.

## Handle Roles with Custom Agents

When Progress Forge composes a prompt with a custom agent, it omits the built-in role prompt. This prevents the built-in persona from being combined with the custom agent's persona.
...
Use `--role` when the workflow should use one of Progress Forge's built-in roles:
...
Progress Forge passes the configured name to the selected AI coding agent but does not verify that the external platform defines it. When the platform reports that an agent is missing, check the following items:
...
1. Confirm that the command uses the intended Progress Forge agent through `--agent` or `agent.default`.
...
```bash
RUST_LOG=debug frg issue draft --custom-agent my-agent
```

Review the workflow trace under `.forge/work/job_*/traces/` when the command creates a job and trace file.

### Configuration Validation Fails

Check the values that Progress Forge validates locally:
...
- Test a custom-agent selection before committing `.forge/config/agents.toml`.
...
|---|---|---|
| `--agent` | Command line | Selects the Progress Forge AI coding agent implementation. |
